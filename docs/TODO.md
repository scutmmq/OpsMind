...
JWT 令牌安全（alg 限制、secret 空值校验、TokenType 校验、标准声明、登录限流）、中间件安全（用户冻结检查、DNS 重绑定防护、X-Request-ID 校验、RBAC 通配权限）、密码策略（utf8 字符计数、unicode 检测一致性）、路由一致性（Auth 路径与 API 文档对齐）、登录审计日志（AuthService.Login 内 slog 记录失败尝试）、Logout token 失效（内存黑名单 + RefreshToken 校验）。
...
（本小节 1 项注释不一致已修复：[rag/types.go](/server/internal/rag/types.go) StepMetric 注释 `hybrid_retrieve` → `vector_retrieve / bm25_retrieve / hybrid_fuse`）

### 文档处理（chunker / parser / processor）

（本小节 11 项已于 2026-06-16 修复：chunker mergeSplits overlap + normalizeText / parser PDF bytes + DOCX 表格+命名空间回退+页面告警 / processor Stop 幂等 + EmbeddingModel 从 KB 读取 + panic recovery + 独立超时 + 重复 indexing）

### 置信度与数据完整性

- 🟡 [service/chat_service.go](/server/internal/service/chat_service.go) — 置信度算法粗糙：直接取 max(chunk.Score)，但 BM25 和 RRF 分数量纲不同
- 🟢 [model/chat.go](/server/internal/model/chat.go) — `ChatMessage.SessionID` 缺少索引

### 服务层集成

- 🟡 [cmd/main.go](/server/cmd/main.go) — config.yaml 的 LLM 模型未同步到 `LLMConfigManager` 作为 fallback
- 🟢 [cmd/main.go](/server/cmd/main.go) — BM25 索引 TTL 30 分钟，新发布的知识文章最多延迟 30 分钟可见
- 🟢 [cmd/main.go](/server/cmd/main.go) — Processor goroutine pool 仅 2 个 worker

### 已修复（6 项，2026-06-16）

CreateChatSession context 传播 / GetChatDetail session.UserID 归属校验 / SubmitFeedback 校验下沉 Service 层 / Question max=2000 + route_count/rerank_count 请求字段 / ChatSessionResponse Pipeline 字段 / PipelineStep 类型定义
...
| 10. 整表空数据 | 2 | 1 | 0 | 0 | 3 |
| 11. P0 覆盖验证 | — | — | — | — | (维护) |
| **合计** | **53** | **71** | **33+9📝** | **51** | **~229** |

> ⭐ 标记项为 2026-06-16 审计新发现（前端 18 项 + 后端 13 项）。
> 📝 标记项为代码与 API 文档/PRD/TECH.md 不一致的文档缺陷（共 9 项）。
> 📌 标记项为代码中已存在的 TODO 注释，本次审计同步收录到对应业务章节（共 53 项，2 项已于 2026-06-16 实现并移除）。
