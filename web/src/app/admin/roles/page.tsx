...
export default function RoleManagePage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editId) { await updateRole(editId, { name, description: desc, permissions: perms }); toast.success('已更新'); }
      else { await createRole({ name, description: desc, permissions: perms }); toast.success('已创建'); }
      setShowDialog(false); mutate();
    } catch (err: unknown) { toast.error(err instanceof Error ? err.message : '保存失败'); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => { try { await deleteRole(deleteId!); toast.success('已删除'); mutate(); } catch (err: unknown) { toast.error(err instanceof Error ? err.message : '删除失败'); } finally { setDeleteId(null); } };
    ...
    <div>
      ...
      <AppleTable
        columns={[
          ...
          { key: 'permissions', title: '权限', render: (r) => <span style={{ fontSize: 12 }}>{r.permissions.join(', ') || '—'}</span> },
          { key: 'actions', title: '', render: (r) => <div style={{ display: 'flex', gap: 4 }}>
            <AppleButton variant="ghost" onClick={() => { setEditId(r.id); setName(r.name); setDesc(r.description || ''); setPerms(r.permissions); setShowDialog(true); }}>编辑</AppleButton>
            <AppleButton variant="utility" onClick={() => setDeleteId(r.id)}>删除</AppleButton>
          </div> },
        ]}
